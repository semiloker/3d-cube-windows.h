...
void DrawCube(HDC hdc, float angleX, float angleY, float cameraX, float cameraY, float cameraZ, int windowWidth, int windowHeight) {
	...
	Vec3 offsets[8] = {
		...
		{0.0f, 0.0f, 0.0f},     // В центрі

		{3.0f, 0.0f, 0.0f},   // Зліва
		{4.0f, -1.0f, 0.0f},    // справаа-зверху
		{4.0f, 0.0f, -1.0f},     // СПРАВА-спереду
		{4.0f, 0.0f, 0.0f}     // В центрі
	};
	...
	for (int j = 0; j < 8; j++) {
		...
		OffsetCube(rotatedVertices, offsets[j].x + cameraX, offsets[j].y + cameraY, offsets[j].z + cameraZ);
